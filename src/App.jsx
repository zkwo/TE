...
import { ShoppingBag, Search, Sparkles, Zap, ShieldCheck, RefreshCw, CheckCircle2, ArrowRight, X } from 'lucide-react';
...
export default function App() {
  ...
  const handleCheckout = async () => {
    ...
    setRobloxUsername('');
    alert(`Pesanan ${selectedItem.name} berhasil dibuat! Admin/Bot akan menghubungi akun ${robloxUsername}.`);
  };
  ...
  return (
    <div className="pb-20">
      {/* Background Animated Dynamic Orbs */}
      <div className="animated-bg">
        <div class="orb orb-1 animate-float-1"></div>
        <div class="orb orb-2 animate-float-2"></div>
      </div>
      ...
      <header className="sticky top-0 z-50 mewah-glass border-b border-white/10 px-4 lg:px-8 py-3.5">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => setIsAdminView(true)} className="px-3 py-1.5 text-xs bg-white/10 border border-white/10 rounded-xl hover:bg-white hover:text-black transition">
              Panel Admin
            </button>
            <button className="flex items-center gap-2 px-4 py-2 text-xs font-bold text-slate-950 bg-white rounded-xl shadow-md">
            ...
            <div key={p.id} className="mewah-card rounded-2xl p-5 border border-white/10 flex flex-col justify-between">
              <div>
                ...
                <div className="w-full aspect-square rounded-xl bg-black/40 flex items-center justify-center p-3 mb-4">
                  <img src={p.image} alt="" className="w-24 h-24 object-contain drop-shadow" />
                </div>
              ...
              <div className="mt-5 pt-3 border-t border-white/10 flex items-center justify-between">
                <div>
                  ...
                  <span class="text-base font-black text-white">Rp{p.price?.toLocaleString('id-ID')}</span>
                </div>
                <button onClick={() => setSelectedItem(p)} className="px-3.5 py-2 rounded-xl bg-white text-slate-950 font-bold text-xs">
                  Beli
                </button>
              </div>
            </div>
      ...
      {selectedItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-xl p-4">
          <div className="mewah-glass w-full max-w-md rounded-3xl p-6 border border-white/20">
            ...
            <button onClick={handleCheckout} className="w-full py-3 bg-white text-slate-950 font-black text-xs rounded-xl">
              BAYAR SEKARANG
            </button>
          </div>
        </div>
